...
<p>The whole stack uses <code>{`syscall`}</code>-based packet handling. It caps at a few hundred kpps on a typical NIC, depending on per-packet overhead. That's fine for functional tests where you're confirming the tunnel terminates and traffic flows. It's not enough for line-rate benchmarks.</p>
<h3 id="dpdk">DPDK</h3>
<p>The DPDK backend shells out to <code>{`fpl-dpdk-c`}</code>, an embedded binary built into the Go executable. The binary is extracted to a temp dir on first use and exec'd with the configured arguments. It uses DPDK's poll-mode drivers to bypass the kernel network stack entirely, going from the userspace few-hundred-kpps ceiling to multi-Mpps line rate.</p>
...
<ul>
<li><strong>Packets in / out</strong> — cumulative counts at the receiver</li>
<li><strong>Bytes in / out</strong> — cumulative volume, reported alongside transmit and receive throughput in bits per second</li>
<li><strong>Round-trip latency on echo</strong> — average, minimum, and maximum time a packet spent in the tunnel plus the receiver</li>
<li><strong>Jitter on echo</strong> — RFC 3550 smoothed jitter (the mean deviation of consecutive round-trip times), the variability metric that matters for real-time traffic</li>
<li><strong>Loss rate</strong> — sent vs echoed; non-zero loss on a clean network indicates UPF, receiver, or tunnel saturation</li>
</ul>
...
<p>To set up and run a user-plane test end to end — starting a receiver, arming <code>{`uplane_start`}</code>, and reading the results — see <Link to="/guides/user-plane-testing">User-plane testing</Link>.</p>
    </DocPage>
  );
}
